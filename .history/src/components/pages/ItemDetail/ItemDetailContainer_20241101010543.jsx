        ...
        <Grid container spacing={2} alignItems="flex-start">
          <Grid item xs={12} sm={6} sx={{ textAlign: 'center' }}>
            <CardMedia
              ...
              image={selectedImage}
              alt={item.title}
              sx={{ objectFit: 'contain', height: 'auto', maxHeight: '300px', borderRadius: '8px' }}
            />
          ...
          <Grid item xs={12} sm={6} sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
            ...
            <Typography variant="body2" sx={{ marginTop: 1 }}>Precio: ${item.price}</Typography>
            <Typography variant="body2" sx={{ marginTop: 1 }}>Stock: {item.stock}</Typography>
            <Box sx={{ marginTop: 2 }}>
              <Button variant="contained" color="primary" onClick={addToCart} sx={{ marginRight: 2 }}>
                Añadir al carrito
              </Button>
