...
const CartContainer = () => {
  const { cart, removeFromCart, updateQuantity, getTotal, getTotalQuantity, addToCart } = useCart();
  ...
  const handleQuantityChange = (prodId, e) => {
    ...
    if (newQuantity >= 1) {
      updateQuantity(prodId, newQuantity); // Actualiza la cantidad solo si es mayor o igual a 1
    }
  };

  // Función para finalizar la compra (solo para demo)
