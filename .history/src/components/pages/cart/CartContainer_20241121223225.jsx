...
const CartContainer = () => {
  ...
  return (
    <Box
      sx={{
        padding: { xs: 3, sm: 5 },  // Ajustar el espaciado según el tamaño de pantalla
    ...
    >
      ...
      {cart.length === 0 ? (
        ...
        <Grid container spacing={4} sx={{ flex: 1 }}>
          {cart.map((product) => (
            <Grid item xs={12} sm={6} md={4} key={product.cartItemId}> {/* Responsividad con grid */}
              <Box
                sx={{
                  display: "flex",
                  flexDirection: "row",
        ...
        <Box sx={{ marginTop: 4 }}>
          ...
          <Box
            sx={{
              display: "flex",
              justifyContent: "space-between",
              flexDirection: { xs: "column", sm: "row" }, // Responsive: en pantallas pequeñas serán columna, en grandes fila
              gap: 2,
            }}
          >
            <Button
