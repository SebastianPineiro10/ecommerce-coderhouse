...
export const CartProvider = ({ children }) => {
    ...
    setCart((prevCart) => {
      ...
      if (productIndex >= 0) {
        ...
        const updatedCart = prevCart.map((item) =>
          item.id === product.id
            ? { ...item, quantity: item.quantity + (product.quantity || 1) } // Aseguramos que la cantidad sea válida
            : item
        );
  ...
  const getTotal = () => {
    return cart.reduce((total, item) => total + (item.price * item.quantity || 0), 0); // Validamos que el cálculo no dé NaN
  };

  // Obtener la cantidad total de productos
  const getTotalQuantity = () => {
    return cart.reduce((total, item) => total + (item.quantity || 0), 0); // Validamos que no dé NaN
  };
