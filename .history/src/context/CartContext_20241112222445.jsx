...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = (item, quantity) => {
    // Asegurarnos de que la cantidad sea un número válido
    const validQuantity = !isNaN(quantity) && quantity >= 1 ? quantity : 1;
    ...
    if (itemRepeat) {
      ...
      const updatedCart = cart.map((i) =>
        i.id === item.id ? { ...i, quantity: i.quantity + validQuantity } : i
      );
      setCart(updatedCart);
      toast.success(`${item.title} ha sido añadido al carrito (Cantidad actualizada)`);
    } else {
      // Si el producto no está en el carrito, lo agregamos con la cantidad válida
      const newCartItem = { ...item, quantity: validQuantity };
  ...
  const updateQuantity = (productId, quantity) => {
    // Asegurarnos de que la cantidad sea un número válido
    if (isNaN(quantity) || quantity < 1) return; // No actualizamos si la cantidad no es válida
