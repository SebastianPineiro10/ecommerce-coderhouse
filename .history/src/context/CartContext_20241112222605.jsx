...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = (item, quantity) => {
    ...
    if (itemRepeat) {
      ...
      // Si el producto no está en el carrito, lo añadimos con un id único generado
      setCart((prevCart) => [
        ...prevCart,
        { ...item, quantity, cartItemId: generateCartItemId() }
      ]);
      toast.success(`${item.title} ha sido añadido al carrito`);
    }
  };
  ...
  const removeFromCart = (productId) => {
    setCart((prevCart) => {
      const updatedCart = prevCart.filter((item) => item.id !== productId); // Eliminamos solo el producto con el id correspondiente
