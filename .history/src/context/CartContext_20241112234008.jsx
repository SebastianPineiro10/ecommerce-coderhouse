...
export const CartProvider = ({ children }) => {
  ...
  // Eliminar producto del carrito usando el cartItemId
  const removeFromCart = (cartItemId) => {
    setCart((prevCart) => {
      // Filtramos el carrito para eliminar solo el producto con el cartItemId específico
  ...
  // Actualizar cantidad de un producto usando el cartItemId
  const updateQuantity = (cartItemId, quantity) => {
    if (quantity < 1 || isNaN(quantity)) return; // Aseguramos que la cantidad sea válida
    setCart((prevCart) => {
      const updatedCart = prevCart.map((item) =>
        item.cartItemId === cartItemId ? { ...item, quantity: quantity } : item  // Usamos cartItemId aquí
      );
