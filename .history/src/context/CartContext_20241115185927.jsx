...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = (product) => {
    ...
    if (isInCart) {
      // Si ya está en el carrito, generar un nuevo array con el producto actualizado
      const updatedCart = cart.map((item) => {
        if (item.id === product.id) {
          return {
            ...item,
            quantity: item.quantity + product.quantity, // Aumentar la cantidad del producto
          };
        }
        return item;
      });
      ...
      // Si no está en el carrito, agregar el producto con la cantidad inicial
