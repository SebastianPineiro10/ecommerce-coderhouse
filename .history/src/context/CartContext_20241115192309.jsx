...
export const CartProvider = ({ children }) => {
    ...
    if (isInCart) {
      ...
      const updatedCart = cart.map((item) => {
        if (item.id === product.id) {
          ...
          return {
            ...item,
            quantity: item.quantity + product.quantity, // Sumar la cantidad
          };
        }
        return item;
      });
      ...
      const newCartItem = {
        ...
        cartItemId: generateCartItemId(), // Asignamos un ID único al carrito
        quantity: product.quantity, // Establecemos la cantidad inicial
      };
  ...
  const updateQuantity = (cartItemId, quantity) => {
    if (quantity < 1 || isNaN(quantity)) return; // Validamos la cantidad
    setCart((prevCart) => {
